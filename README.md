...
`setRegister(_:to:)`, `digitalRead(into:pin:)`, `analogRead(into:channel:)`,
`ifTrue(_:_:_:then:elseDo:)` (operands `.reg(0...15)`/`.number`, ops `== != < > <= >=`).
...
* Handles: `BODY_GEN` (`0x22`) reads the response generation (++ per request).
  `SNAPSHOT` (`0x23`) copies a value into one of the 12 grow-only slots (2 JSON + 10 string) that outlive the
